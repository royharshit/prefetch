...
uint32_t time_prefetch_h1(uint32_t *data, int fd) {
    uint32_t sum = 0; 
    for (int i = 0; i < elems; i++) {
...
uint32_t time_nonprefetch_h2(uint32_t *data, int fd) {
    uint32_t sum = 0; 
    for (int i = 0; i < elems; i++) {
...
uint32_t time_prefetch_h2(uint32_t *data, int fd) {
    uint32_t sum = 0; 
    for (int i = 0; i < elems; i++) {
        __builtin_prefetch(&data[i + DISTANCE]);
    	sum += hash(hash(data[i]));
...
uint32_t time_nonprefetch_h3(uint32_t *data, int fd) {
    uint32_t sum = 0; 
    for (int i = 0; i < elems; i++) {
...
uint32_t time_prefetch_h3(uint32_t *data, int fd) {
    uint32_t sum = 0; 
    for (int i = 0; i < elems; i++) {
        __builtin_prefetch(&data[i + DISTANCE]);
    	sum += hash(hash(hash(data[i])));
...
uint32_t time_nonprefetch_h4(uint32_t *data, int fd) {
    uint32_t sum = 0; 
    for (int i = 0; i < elems; i++) {
...
uint32_t time_prefetch_h4(uint32_t *data, int fd) {
    uint32_t sum = 0; 
    for (int i = 0; i < elems; i++) {
        __builtin_prefetch(&data[i + DISTANCE]);
    	sum += hash(hash(hash(hash(data[i]))));
...
uint32_t time_nonprefetch_h5(uint32_t *data, int fd) {
    uint32_t sum = 0; 
    for (int i = 0; i < elems; i++) {
...
uint32_t time_prefetch_h5(uint32_t *data, int fd) {
    uint32_t sum = 0; 
    for (int i = 0; i < elems; i++) {
        __builtin_prefetch(&data[i + DISTANCE]);
    	sum += hash(hash(hash(hash(hash(data[i])))));
...
int main() {
    ...
    pe.type = PERF_TYPE_RAW;
    pe.size = sizeof(struct perf_event_attr);
    pe.config = CYCLES_WITH_DATA_RD_EVENT | (CYCLES_WITH_DATA_RD_UMASK << 8) | (CYCLES_WITH_DATA_RD_CMASK << 16);
    pe.disabled = 1; 
    pe.exclude_kernel = 1;
    pe.exclude_hv = 1;
    pe.read_format = PERF_FORMAT_GROUP | PERF_FORMAT_ID;
    ...
    int32_t N = 1;
		...
		ioctl(fd1, PERF_EVENT_IOC_RESET, PERF_IOC_FLAG_GROUP);
            	...
            	sum1 = time_nonprefetch_h2(data, fd1);
            else if (N == 3)
            	sum1 = time_nonprefetch_h3(data, fd1);
            else if (N == 4)
            	sum1 = time_nonprefetch_h4(data, fd1);
            else if (N == 5)
            	sum1 = time_nonprefetch_h5(data, fd1);

            if (N == 1)
            	sum2 = time_prefetch_h1(data, fd1);
            else if (N == 2)     
            	sum2 = time_prefetch_h2(data, fd1);
            else if (N == 3)
            	sum2 = time_prefetch_h3(data, fd1);
            else if (N == 4)
            	sum2 = time_prefetch_h4(data, fd1);
            else if (N == 5)
            	sum2 = time_prefetch_h5(data, fd1);
